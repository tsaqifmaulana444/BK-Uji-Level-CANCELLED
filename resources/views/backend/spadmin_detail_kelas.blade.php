...
@section('isi')
        ...
        <div class="w-fit mx-auto mt-20">
            <iframe src="https://giphy.com/embed/l1J9Li0X2Ib1VbLmo" width="480" height="270" frameBorder="0" class="giphy-embed" allowFullScreen></iframe>
            <p class="w-fit mx-auto mt-4">Maaf, Data Pada Halaman Ini Kosong Seperti Prestasi Spurs</p>
            <div class="w-fit mx-auto underline">
                <a href="{{ route('spadmin.create_murid', [$kelas, $id]) }}">Tambah Data</a>
            </div>
        </div>
    @else
        <div class="flex ml-4 justify-between">
            ...
            <a href="{{ route('spadmin.create_murid', [$kelas, $id]) }}">
                <button type="button"
                    class="text-white bg-red-700 hover:bg-red-800 font-medium rounded-lg text-sm px-4 py-2 mr-[5.3vw]">Tambah
                    Murid</button>
            </a>
                                    ...
                                    @foreach ($datas as $data)
                                        <tr>
                                            <td
                                                ...
                                                <span class="text-xs font-semibold leading-tight"> {{ $i++ }}
                                                </span>
                                            </td>
                                            <td
                                                class="p-2 text-sm leading-normal text-left align-middle bg-transparent border-b whitespace-nowrap">
                                                <span class="text-xs font-semibold leading-tight"> {{ $data->name }}
                                                </span>
                                            </td>
                                            <td
                                                class="p-2 text-sm leading-normal text-left align-middle bg-transparent border-b whitespace-nowrap">
                                                <span class="text-xs font-semibold leading-tight"> {{ $data->email }}
                                                </span>
                                            </td>
                                            <td
                                                class="p-2 text-sm leading-normal text-left align-middle bg-transparent border-b whitespace-nowrap">
                                                <span class="text-xs font-semibold leading-tight"> {{ $data->no_identitas }}
                                                </span>
                                            </td>
                                            <td
                                                class="p-2 text-sm leading-normal text-left align-middle bg-transparent border-b whitespace-nowrap flex">
                                                <form id="deleteForm{{ $data->id }}"
                                                    action="/super_admin/murid/delete/{{ $data->id }}" method="POST">
                                                    @csrf
                                                ...
                                                <a href="{{ route('spadmin.edit_murid', $data->id) }}">
                                                    <button type="button"
                                                        class=" text-white bg-green-700 hover:bg-green-800 font-medium rounded-lg text-sm px-1 py-1">
                                                        <span class="material-symbols-outlined mt-1 w-9">
                                                            edit
                                                        </span>
                                                    </button>
